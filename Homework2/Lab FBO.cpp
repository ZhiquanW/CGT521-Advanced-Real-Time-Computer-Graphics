...
bool edge = false;
glm::vec3 pos(2.0f, -2.0f, 7.0f);
glm::vec3 target(2.0f, -2.0f, 0.0f);
void draw_gui() {
	ImGui_ImplGlut_NewFrame();
	ImGui::Begin("Debug");
	ImGui::Checkbox("Edge Detection", &edge);
...
void draw_pass_1() {
	...
	glm::mat4 V = glm::lookAt(pos, target, glm::vec3(0.0f, 1.0f, 0.0f));
	glm::mat4 P = glm::perspective(40.0f, (float)win_width / win_height, 0.1f, 100.0f);
	...
	int pick_loc = glGetUniformLocation(shader_program, "picked");
	if (pick_loc != -1) {
		glUniform1i(pick_loc, picked_id]); // we bound our texture to texture unit 0
	}
	glBindVertexArray(mesh_data.mVao);
	glDrawElementsInstanced(GL_TRIANGLES, mesh_data.mNumIndices, GL_UNSIGNED_INT, 0, 12);
...
void draw_pass_2()
{
   ...
   int edge_loc = glGetUniformLocation(shader_program, "edge");
   if (edge_loc != -1) {
	   glUniform1f(edge_loc, edge); // we bound our texture to texture unit 0
   }
...
void display()
{
   ...
   glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); //Clear the back buffer
   draw_pass_2();
   glViewport(0, 0, win_width, win_height); //Render to the full viewport
...
void mouse(int button, int state, int x, int y) {
	...
	if (state == 0) {
		...
		glReadPixels(x, glutGet(GLUT_WINDOW_HEIGHT) - y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, buffers);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		picked_id = float(buffers[0])/21.0f;
		//picked_id = int(float(buffers[0]) * 12.0f / 255.0f );
		std::cout << "ID: " << picked_id << std::endl;
	}
	glutPostRedisplay();
}
...
void initOpenGl()
{
   ...
   const int scr_width = glutGet(GLUT_SCREEN_WIDTH);
   const int scr_height = glutGet(GLUT_SCREEN_HEIGHT);

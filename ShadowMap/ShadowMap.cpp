...
void draw_pass_1() //draw from light point-of-view
{
   ...
   glPolygonOffset(0.5f,-0.5f); //No offset is being applied when params are 0.0, 0.0. Try changing these numbers to fix the shadow map "acne" problem
...
void draw_pass_2()
{
   ...
   const int PCF_loc = glGetUniformLocation(shader_program, "PCF");
   if (PCF_loc != -1) {
	   glUniform1i(PCF_loc, PCF);
   }
   const int w_loc = glGetUniformLocation(shader_program, "softness");
   if (w_loc != -1) {
	   glUniform1f (w_loc, w);
	   std::cout << w << std::endl;
   }
   const int display_lit4_loc = glGetUniformLocation(shader_program, "display_lit4");
   if (display_lit4_loc != -1) {
	   glUniform1i(display_lit4_loc, display_lit4);
   }
   draw_scene(V_cam);
}
